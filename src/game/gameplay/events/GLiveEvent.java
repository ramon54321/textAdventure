...
/*
        ...
        bar.actionNodes.get(0).
                setAction(
                        () -> {

                            System.out.println("Hello, Ramon.");

                            bar.actionNodes.get(0).setText("I am not Ramon");
                            bar.actionNodes.get(0).setAction(
                                    () -> {
                                        System.out.println("But you just said you were! I'm not talking to you any more.");
                                        bar.actionNodes.remove(0);
                                        setFirstToSecond(currentObject, bar);
                                    }
                            );

                            bar.actionNodes.get(1).setAction(
                                    () -> {
                                        System.out.println("Goodbye, Ramon.");
                                        System.out.println("System will exit now");

                                        System.exit(3_1415);
                                    }
                            );
                            setFirstToSecond(currentObject , bar); // wire the story back to Fork "bar"

                        }
                );
        bar.actionNodes.get(1).
                setAction(
                        () -> {
                            System.out.println("Goodbye to you to.");
                            System.out.println("System will exit now");
    ...
    public void setFirstToSecond (GNavigator first, GNavigator second) {
        first = second ;
    }
